...
public class LampEditor {
    ...
    public LampEditor(LampManager lampManager, RoomManager roomManager) {
    ...
    public void openAddDialog() {
        Dialog dialog = createLampDialog("Nouvelle Lampe", null);
        dialog.open();
    }

    public void openEditDialog(Lamp lamp) {
        if (lamp != null) {
            Dialog dialog = createLampDialog("Modifier la Lampe", lamp);
            dialog.open();
        } else {
            showWarningNotification("Veuillez sélectionner une lampe à modifier");
        }
    }

    public void confirmDelete(Lamp lamp) {
        if (lamp != null) {
            ConfirmDialog confirmDialog = new ConfirmDialog();
            confirmDialog.setHeader("Confirmer la suppression");
            confirmDialog.setText("Êtes-vous sûr de vouloir supprimer la lampe \"" + 
                                 lamp.getCustomName() + "\" ? Cette action est irréversible.");
            ...
            confirmDialog.addConfirmListener(e -> {
                try {
                    lampManager.deleteById(lamp.getId());
                    showSuccessNotification("Lampe supprimée avec succès !");
            ...
            showWarningNotification("Veuillez sélectionner une lampe à supprimer");
        }
    }

    private Dialog createLampDialog(String title, Lamp lamp) {
        ...
        // Champs de saisie
        TextField nameField = new TextField("Nom de la lampe");
        nameField.setPlaceholder("Entrez le nom de la lampe...");
        ...
        if (lamp != null) {
            nameField.setValue(lamp.getCustomName() != null ? lamp.getCustomName() : "");
            roomComboBox.setValue(lamp.getRoom());
            posXField.setValue(lamp.getPosX());
            posYField.setValue(lamp.getPosY());
            posZField.setValue(lamp.getPosZ());
        }
        ...
        // Boutons
        Button saveButton = new Button(lamp == null ? "✅ Créer" : "💾 Sauvegarder", 
            new Icon(lamp == null ? VaadinIcon.CHECK : VaadinIcon.DOWNLOAD));
            ...
            if (validateFields(nameField, roomComboBox, posXField, posYField, posZField)) {
                try {
                    if (lamp == null) {
                        ...
                        setLampFields(newLamp, nameField, roomComboBox, posXField, posYField, posZField);
                        lampManager.save(newLamp);
                        showSuccessNotification("Lampe créée avec succès !");
                    } else {
                        setLampFields(lamp, nameField, roomComboBox, posXField, posYField, posZField);
                        lampManager.save(lamp);
                        showSuccessNotification("Lampe modifiée avec succès !");
                    }
    ...
    private void setLampFields(Lamp lamp, TextField nameField, ComboBox<Room> roomComboBox,
                              NumberField posXField, NumberField posYField, NumberField posZField) {
        lamp.setCustomName(nameField.getValue().trim());
        lamp.setRoom(roomComboBox.getValue());
        lamp.setPosX(posXField.getValue());
        lamp.setPosY(posYField.getValue());
        lamp.setPosZ(posZField.getValue());
    }
